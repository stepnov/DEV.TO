...
/* Delete item */
const deleteItem = () => {
  articlesStore.deleteItem(currentId.value)
}

const items = computed(() => articlesStore.data)
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'author' && props.sortDirection]"
        @click="sort('author')"
      >Author</th>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'featured' && props.sortDirection]"
        @click="sort('featured')"
      >Featured</th>

      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'images' && props.sortDirection]"
        @click="sort('images')"
      >Images</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/articles/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        />
              ...
              <td data-label="author">
                {{ dataFormatter.usersOneListFormatter(client.author) }}
              </td>

              <td data-label="category">
                {{ dataFormatter.categoriesOneListFormatter(client.category) }}
              </td>
              ...
              <td data-label="featured">
                {{ dataFormatter.booleanFormatter(client.featured) }}
              </td>                                                      

              <td data-label="images">
                <UserAvatar v-if="!client.images?.length"
                  class="w-10 h-10 md:mr-6"
                />
                <img v-if="client.images.length"
                  class="w-10 h-10 md:mr-6 rounded-full"
                  :src="client.images[0].publicUrl"
                  :alt="client.images[0].name"
                />
              </td>
