  ...
  <li>
    ...
    >
      <BaseIcon
        ...
        :path="item.icon"
        class="flex-none"
        :class="[ 'menu-item-icon', vSlot && vSlot.isExactActive ? styleStore.asideMenuItemActiveStyle : styleStore.asideMenuItemInactiveStyle ]"
        w="w-12"
      />
      <span
        class="grow"
        :class="[ 'menu-item-link', vSlot && vSlot.isExactActive ? styleStore.asideMenuItemActiveStyle : styleStore.asideMenuItemInactiveStyle ]"
      >{{ item.label }}</span>
      <BaseIcon
        v-if="hasDropdown"
