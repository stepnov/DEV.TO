...
import TableSampleComments from '@/components/Comments/TableSampleComments.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Text', title: 'text'},

          {label: 'Author', title: 'author'},{label: 'Article', title: 'article'},];
...
const deleteFilter = (index) => {
  ...
  generateQuery();
  commentsStore.fetch(null, query.value);
}
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Comments'
    : 'http://localhost:8080/api-docs/#/Comments';
});
...
<template>
  ...
  <SectionHeroBar>Comments</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for comments</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleComments checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
