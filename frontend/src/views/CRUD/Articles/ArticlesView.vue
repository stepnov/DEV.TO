...
import TableSampleArticles from '@/components/Articles/TableSampleArticles.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Title', title: 'title'},{label: 'Body', title: 'body'},

          {label: 'Author', title: 'author'},{label: 'Category', title: 'category'},];
...
const deleteFilter = (index) => {
  ...
  generateQuery();
  articlesStore.fetch(null, query.value);
}
...
<template>
  ...
  <SectionHeroBar>Articles</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for articles</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleArticles checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
